...
我在第2篇文章[《日志系统：一条SQL更新语句是如何执行的？》](https://bookstack.xnzone.eu.org/01-mysql-45/02-how-sql-execut/)中，和你讲到binlog（归档日志）和redo log（重做日志）配合崩溃恢复的时候，用的是反证法，说明了如果没有两阶段提交，会导致MySQL出现主备数据不一致等问题。
...
接下来，我再和你分享@ithunter 同学在第8篇文章[《事务到底是隔离的还是不隔离的？》](https://bookstack.xnzone.eu.org/01-mysql-45/08-transation-isolate/)的评论区提到的跟索引相关的一个问题。我觉得这个问题挺有趣、也挺实用的，其他同学也可能会碰上这样的场景，在这里解答和分享一下。
...
> 知识点在[《行锁功过：怎么减少行锁对性能的影响？》](https://bookstack.xnzone.eu.org/01-mysql-45/07-lock-row/)  
